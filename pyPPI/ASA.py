...
class ASA(object):
    ...
    def nearAtomsOnChain(chain, atoms, ktree):
        ...
        for atom in [atom for atom in atoms if atom.chain == chain]:
            ...
            for atom2 in ktree.findByDistance(query_point=atom.coord,
                                              distance=(radio_atom(atom.atomType) + extraRad) ** 2):
                if atom2 != atom:
    ...
    def print_point(self, points):
        ...
        for x, y, z in points:
            self.q_points += 1
            point_to_print = 'HETATM {0:^4}  O   HOH I {1:^4} {2:^11.3f} {3:^4.3f} {4:^8.3f}  0.83 56.58           O '.format(
...
def spiral(r, atom, n_points=SPIRAL_POINTS):
    ...
    move = 0
    z = 1 - (1.0 / n_points)
    zMove = (float(2) / n_points)
    mvPlus = (pi * (3 - sqrt(5)))
    for k in range(0, n_points):
        rUnit = sqrt(1 - z * z)
        xCoord = (cos(move) * rUnit) * r + atom.x
        yCoord = (sin(move) * rUnit) * r + atom.y
        zCoord = z * r + atom.z
        move += mvPlus
        z -= zMove
